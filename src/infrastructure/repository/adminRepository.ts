...
class AdminRepository implements AdminRepo {
    ...
    async getRequest(limit: number, skip: number): Promise<paginationBeneficiary> {
        ...
        return { request: request, totalPages: totalPages }
    ...
    async getallReports(): Promise<PostReport[] | null> {
        const allReport = await PostReportModel.find({}).populate('postId').populate('userId').exec()
        return allReport
    ...
    async getFundRequest(limit: number, skip: number): Promise<paginationBeneficiary | undefined> {
        try {
            ...
            return { request: getFundRequest, totalPages: totalPages }
